// ==UserScript==
// @name         onlylady_Spider
// @version      1.0
// @author       Mr.Robot
// @include       http://bbs.onlylady.com/search.php?*
...
function() {
    ...
    $(window).on("load", function() {
        ...
        if (localStorage.getItem("Index") == null || (location.href.search("page=")==-1)) {
        ...
        $("#threadlist").find("ul").find("li").each(function() {
            var title = $(this).find(".xs3").find("a").contents().text();
            var url = $(this).find(".xs3").find("a").attr("href");
            // var brief_of_content =
            var brief_of_content, time, account, tieba;
            $(this).find("p").each(function(i, value){
                if (i == 1) {
                    brief_of_content = $(this).contents().text();
                }else if(i == 2){
                    $(this).find("span").each(function(j, value){
                        switch(j){
                            case 0:
                                time = $(this).contents().text();
                                break;
                            case 1:
                                account = $(this).contents().text();
                                break;
                            case 2:
            ...
            console.log(keyword_from_search+"@"+title+"@"+typeof(brief_of_content)+"@"+url+"@"+account+"@"+tieba+"@"+time);
