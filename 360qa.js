...
// @version      0.1
// @author       Mr.Robot
// @include       http://wenda.so.com/search/?*
...
function() {
    ...
    $(window).on("load", function() {
        ...
        var keyword_from_search = $("#js-sh-ipt").attr("value");
        console.log(keyword_from_search);
        $(".qa-list").find(".item").each(function() {
            var title = $(this).find("a").contents().text();
            var url = "http://wenda.so.com" + $(this).find("a").attr("href");
            var brief_of_content = $(this).find(".qa-i-bd").contents().text();
            var account = $(this).find(".qa-i-ft").find(".cate").find("a").contents().text();
            var time = $(this).find(".qa-i-ft").contents().text().split("提问时间: ")[1];
            var hash = {
        ...
        if ($(".next").contents().text() != "") {
            ...
            if (confirm_save) {
                var file_name = "360-" + keyword_from_search + ".json";
