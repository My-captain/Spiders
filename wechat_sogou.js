// ==UserScript==
// @name         weChatText&ImageSpider
// @version      0.1
// @author       Mr.Robot
// @include       http://weixin.sogou.com/weixin*
...
function() {
    ...
    $(window).on("load", function() {
        ...
        var keyword_from_search = $("#query").eq(0).val();

        $("#main").find(".news-list").find("li").each(function() {
            // 内容部分
            // var $content = $(this).find(".content");
            var $result_li = $(this);
            var $txt_box = $result_li.find(".txt-box");
            var $img_box = $result_li.find(".img_box");

            var title = $txt_box.find("h3").find("a").contents().text();
            var brief_of_content = $txt_box.find(".txt-info").contents().text();
            var $info_div = $txt_box.find(".s-p");
            var account = $info_div.find(".account").contents().text();
            var time = $info_div.find(".s2").find("script").contents().text();
            var url = $txt_box.find("h3").find("a").attr("href");
            console.log(title);
            console.log(brief_of_content);
            console.log(account);
            console.log(time);
        ...
        function click_to_next() {
            document.getElementById("sogou_next").click();
        }
